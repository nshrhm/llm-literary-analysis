...
def main():
    parser = argparse.ArgumentParser(description="Run Llama batch experiments")
    parser.add_argument("--model", nargs="+", choices=list(LLAMA_MODELS.keys()), help="Specify one or more models to run")
    ...
    if args.model:
        selected_models = {model: LLAMA_MODELS[model] for model in args.model}
    else:
        selected_models = LLAMA_MODELS
